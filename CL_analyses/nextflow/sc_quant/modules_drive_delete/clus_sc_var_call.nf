process clus_sc_var_call {
    ...
    """
    #!/bin/bash
    cp ${sample}_clus${clus}_filt.recode.vcf snps.vcf
    gzip snps.vcf
    ref_genome=${params.fasta_dir}/${ref}.fna

    #Filter bam for only barcoded reads
    samtools index ${sample}_dup_NCR_clus${clus}.bam
    samtools view -b -d CB -h -q 30 ${sample}_dup_NCR_clus${clus}.bam > CB_subset.bam
    ...
    join reads_barcodes.txt snps_sam2tsv.tsv | gzip > ${species}_${sample}_clus${clus}_fin_snp_read.txt.gz
