process get_refs {
    ...
    awk 'BEGIN{FS=OFS="/";filesuffix="genomic.fna.gz"}{ftpdir=\$0;asm=\$10;file=asm"_"filesuffix;print "rsync -t -v "ftpdir,file" ./"}' ftp_links.txt | sed 's/https/rsync/g' > download_gnom_files.sh

    source download_gff_files.sh
    source download_cds_files.sh
    source download_pro_files.sh
    source download_gnom_files.sh

    gunzip *gz
    mv *genomic.gff ${species}.gff
    mv *cds_from_genomic.fna ${species}.cds.fna
    mv *protein.faa ${species}.protein.faa
    mv *genomic.fna ${species}.fna
