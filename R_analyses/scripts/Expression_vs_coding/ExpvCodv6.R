...
PBDGE <- function(label, sce, func = 'custom', seurat_obj = seurat_integrated, consistent_genes = T){
  if (consistent_genes == TRUE){
    cells <- filter(seurat_integrated@meta.data, celltype == label) %>% 
      dplyr::select(cells) %>% c() %>% unlist()
    keep <- (rowSums(seurat_integrated@assays$RNA$counts[,cells] > 1) > (0.05 * length(cells))) #| 
...
for (ct in unique(agg_cell_types$celltype)){
  keep1 <- cpm[,y$samples$celltype== ct] >= 2
...
tau_keep_genes <- list()
for (ct in unique(seurat_integrated$celltype)){
  counts <- seurat_integrated@assays$RNA$counts
  cells <- filter(seurat_integrated@meta.data, celltype == ct) %>% 
    dplyr::select(cells) %>% c() %>% unlist()
  keep <- rowSums(counts[,cells] > 1) > (0.05 * length(cells))
  keep_genes <- names(keep)[keep]
  tau_keep_genes[[ct]] <-  keep_genes
}
...
og_subset_gene_taus <- merge(og_subset, taus, by.x = 'Anas_platyrhynchos_gene', by.y = 'gene', all = T) %>% 
  ...
  mutate(top_celltype = ifelse(tau < 0.8, "universal", top_celltype)) %>% 
  filter(dS <=2)
...
for (ct in unique(og_subset_gene_taus$top_celltype)){
  ...
  for (i in 1:1000){
    gene_sample <- genes %>%sample(., 10, replace = F) %>% unique()
